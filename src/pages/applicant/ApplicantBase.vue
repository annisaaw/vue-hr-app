    ...
    <div class="bg-black-alt font-sans leading-normal tracking-normal">
      ...
      <div class="px-8 container w-full mx-auto flex mt-6 flex m-5 justify-between items-center">
			...
			<div class="flex">
				...
				<router-link v-if="isEmployer" :to="{name: 'add_applicant'}" class="bg-yellow-500 hover:bg-yellow-600 text-gray-700 py-2 text-sm px-4 rounded m-1">Add New Applicant</router-link>
			</div>
      </div>
						...
						<table class="min-w-full">
							<tbody class="bg-white">
								...
								<tr class="hover:bg-gray-100" v-for="(item, idx) in filterApplicant(allApplicant)" :key="idx">
									...
									<td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200" 
										v-if="activeIndex === 2" @click="approved(item.name)">
										<div class="text-grey-300 bg-yellow-500 px-2 py-1 text-xs rounded-lg inline-block">
